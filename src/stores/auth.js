...
export function useAuth() {
  ...
  // Avatar URL — gravatar style based on email
  const avatarUrl = computed(() => {
    if (!state.user) return ''
    return `https://i.pravatar.cc/200?u=${encodeURIComponent(state.user.email || '')}`
  })
  ...
  return { user, isAuthenticated, login, register, logout, enrollCourse, updateUser, avatarUrl }
}
