      ...
      <div class="flex items-center justify-between h-14 md:h-[4.25rem]">
        ...
        <div class="flex items-center gap-1 md:gap-2">
          ...
          <template v-if="auth.isAuthenticated()">
            <div class="relative hidden md:block" ref="profileRef">
              <button type="button" @click="profileOpen = !profileOpen" class="profile-avatar-btn">
                <img :src="'https://i.pravatar.cc/200?u=' + (auth.user.value?.email || 'default')" class="w-10 h-10 rounded-full border-[3px] border-blue-500 hover:scale-105 transition-transform duration-200 object-cover" />
              </button>
              <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 -translate-y-2 scale-95" enter-to-class="opacity-100 translate-y-0 scale-100" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100 translate-y-0 scale-100" leave-to-class="opacity-0 -translate-y-2 scale-95">
                <div v-if="profileOpen" class="absolute top-full right-0 mt-3 w-[280px] bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-50">
                  <!-- Dropdown header -->
                  <div class="px-4 py-4 bg-gradient-to-br from-blue-600 to-blue-700 text-white flex items-center gap-3">
                    <img :src="'https://i.pravatar.cc/200?u=' + (auth.user.value?.email || 'default')" class="w-12 h-12 rounded-full border-2 border-white/50 object-cover" />
                    <div class="min-w-0">
                      <div class="font-bold text-[15px] truncate">{{ auth.user.value?.firstName }} {{ auth.user.value?.lastName }}</div>
                      <div class="text-xs text-white/80 truncate">{{ auth.user.value?.email }}</div>
                    </div>
                  </div>
                  <!-- Dropdown items -->
                  <RouterLink to="/profile" @click="profileOpen = false" class="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-all duration-150">
                    <User class="w-[18px] h-[18px]" /> Profil
                  </RouterLink>
                  <RouterLink to="/my-learning" @click="profileOpen = false" class="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-all duration-150">
                    <BookOpen class="w-[18px] h-[18px]" /> Kurslarym
                  </RouterLink>
                  <div class="h-px bg-gray-100 mx-3"></div>
                  <button type="button" @click="handleLogout; profileOpen = false" class="w-full flex items-center gap-3 px-4 py-3 text-sm text-red-500 hover:bg-red-50 transition-all duration-150">
                    <LogOut class="w-[18px] h-[18px]" /> Çykyş
                  </button>
                </div>
              </Transition>
            </div>
